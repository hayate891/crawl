...
-- simulate drinking `iterations` mutation potions in a row, a bunch of times (determined by `tries`)
-- this is mostly useful when looking at the output, doesn't really do much that test_random_mutations doesn't
local function test_potion(tries, iterations)
	for i=1, tries do
		you.delete_all_mutations("mutation test")
		assert(you.how_mutated(true, true, true) == 0, "Clearing mutations failed, currently: " .. you.mutation_overview())
		for j=1, iterations do
			simulate_mutation_pot()
			-- crawl.stderr("iter " .. j .. ", " .. you.how_mutated(true, true, true) .. " mutations. result: " .. you.mutation_overview() .. eol)
		end
		if not silent then
			crawl.stderr("Potion test try " .. i .. ", " .. you.how_mutated(true, true, true) .. " mutations. result: " .. you.mutation_overview() .. eol)
		end
	end
end
...
local function test_random_mutations(tries, iterations, chance_temporary, chance_clear)
	for i=1, tries do
		...
		for j=1, iterations do
			if crawl.x_chance_in_y(chance_clear, 100) then
				if crawl.coinflip() then
					...
					assert(you.temp_mutations() == 0, "Clearing temporary mutations failed, currently at " .. you.temp_mutations() .. ": " .. you.mutation_overview())
				else
		...
		if not silent then
			crawl.stderr("Random test try " .. i .. ", " .. you.how_mutated(true, true, true) .. " mutations. result: " .. you.mutation_overview() .. eol)
		end
	end
end
